...
보유한 장비 아이템을 시각적 그리드로 확인할 수 있는 보기 전용 페이지입니다.

- **3탭 구성**: 의상(body), 착용 소품(body_item), 손 아이템(hand_item)
- **의상 하위 필터**: casual, formal, sporty, outerwear, bowtie, SVG 카테고리별 필터링
- **정렬 옵션**: 이름순(알파벳) / 획득순(기본 의상 > 출석 보상 > 일일 보상)
- **해금/잠금 구분**: 잠긴 아이템은 grayscale + 자물쇠 아이콘으로 표시
- **획득 경로 배지**: "기본 의상" / "출석 보상" / "일일 보상" 라벨
- **반응형 그리드**: 화면 크기에 따라 3~6열 적응형 레이아웃
- **Read-Only**: Firestore 쓰기 없이 기존 데이터 읽기만 수행
...
| 아이템 인벤토리 | `/inventory` | 보유 아이템 그리드 + 탭/필터/정렬 + 해금 상태 표시 |
...
│   ├── inventory/page.tsx          # 아이템 인벤토리 (옷장) 페이지
...
│   │   ├── InventoryGrid.tsx      # 아이템 인벤토리 그리드 컴포넌트
